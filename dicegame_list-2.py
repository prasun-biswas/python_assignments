...
class Dicegame:
    def __init__(self):
        ...
        self.__lockbuttons = []
        for i in range(DICE_NUMBER):
            new_button = Button(self.__window, command=lambda i=i: self.change_dice_status(i))
    ...
    def change_dice_status(self, i):
        if self.__dices_in_use[i]:
            self.__dices_in_use[i] = False
            self.__lockbuttons[i].configure(text="locked")
            self.__lockbuttons[i].configure(background="red")
        else:
            self.__dices_in_use[i] = True
            self.__lockbuttons[i].configure(text="unlocked")
            self.__lockbuttons[i].configure(background="green")
